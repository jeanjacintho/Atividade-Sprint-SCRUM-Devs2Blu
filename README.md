# Atividade-Sprint-SCRUM-Devs2Blu

**Nome integrantes:** Jean Carlo Massing Jacintho.  
**Tecnologias usadas:** `React` com `Vite`, `Electron`, `Mysql`, `Typescript`.  
**Descrição do projeto:** O Yuno é uma aplicação desktop robusta projetada para gerenciar e reproduzir cursos em vídeo armazenados localmente, oferecendo uma experiência de aprendizagem completa e personalizada com recursos avançados de acompanhamento e gamificação.  
**Quantidade total de Sprints:** 25   
**Resumo sobre vantagens:**   

## SPRINT 1: Configuração do ambiente e estrutura base  
- **Descrição:** Configuração inicial do projeto React com Vite, integração com Electron, configuração do banco MySQL e estrutura básica de pastas.  
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `3 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 05/08/2025 a 11/08/2025  
- **Observações:**

## SPRINT 2: Sistema de Seleção de Pastas  
- **Descrição:** Implementação da funcionalidade para selecionar pasta de cursos usando API do Electron, validação de estrutura de pastas.  
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 12/08/2025 a 18/08/2025  
- **Observações:**

## SPRINT 3: Modelagem e Criação do Banco de Dados  
- **Descrição:** Criação das tabelas do banco (usuários, cursos, aulas, progresso, comentários, avaliações), scripts de migração .
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `3 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 19/08/2025 a 25/08/2025 
- **Observações:**

## SPRINT 4: Escaneamento e Catalogação de Cursos 
- **Descrição:** Sistema para escanear pasta selecionada, identificar cursos e aulas, salvar informações no banco de dados.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 26/08/2025 a 03/09/2025
- **Observações:**

## SPRINT 5: Interface Principal - Lista de Cursos 
- **Descrição:** Desenvolvimento da tela principal com grade de cursos, exibição de thumbnails, informações básicas dos cursos.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 04/09/2025 a 10/09/2025
- **Observações:**

## SPRINT 6: Navegação Interna dos Cursos
- **Descrição:** Interface para visualizar estrutura do curso, lista de aulas, organização por módulos/seções.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 11/09/2025 a 17/09/2025
- **Observações:**

## SPRINT 7: Player de Vídeo Básico
- **Descrição:** Implementação do player de vídeo com controles básicos (play, pause, volume, timeline).
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 8/09/2025 a 26/09/2025
- **Observações:**

## SPRINT 8: Sistema de Registro de Progresso
- **Descrição:** Funcionalidade para salvar progresso das aulas, marcar aulas como assistidas, persistir tempo de visualização.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 27/09/2025 a 03/10/2025
- **Observações:**

## SPRINT 9: Funcionalidade "Continuar de Onde Parei"
- **Descrição:** Sistema para retomar aulas no ponto exato onde foram pausadas, interface de "continuar assistindo".
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `3 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 04/10/2025 a 10/10/2025
- **Observações:**

## SPRINT 10: Sistema de Avaliação
- **Descrição:** Interface para adicionar, editar e excluir comentários nas aulas, exibição de comentários por timestamp.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 11/10/2025 a 17/10/2025
- **Observações:**

## SPRINT 11: Sistema de Comentários
- **Descrição:** Interface para adicionar, editar e excluir comentários nas aulas, exibição de comentários por timestamp.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `3 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 18/10/2025 a 24/10/2025
- **Observações:**

## SPRINT 12: Sistema de Favoritos
- **Descrição:** Funcionalidade para favoritar/desfavoritar cursos e aulas, lista de favoritos, filtros.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `3 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 25/10/2025 a 31/10/2025
- **Observações:**

## SPRINT 13: Visualização de Progresso dos Cursos
- **Descrição:** Interface para mostrar progresso individual de cada curso, barras de progresso, estatísticas de conclusão.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 01/11/2025 a 07/11/2025
- **Observações:**

## SPRINT 14: Planejamento de Estudos - Configuração Básica
- **Descrição:** Interface para definir metas de estudo (horas por dia, dias da semana), armazenamento de configurações.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 08/11/2025 a 14/11/2025
- **Observações:**

## SPRINT 15: Dashboard de Planejamento de Estudos
- **Descrição:** Painel para visualizar progresso do planejamento, calendário de estudos, metas diárias/semanais.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 15/11/2025 a 23/11/2025
- **Observações:**

## SPRINT 16: Sistema de Gamificação - Pontuação Básica
- **Descrição:** Sistema de pontos por aulas assistidas, conclusão de cursos, sequências de estudo.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 24/11/2025 a 30/11/2025
- **Observações:**

## SPRINT 17: Sistema de Gamificação - Conquistas e Badges
- **Descrição:** Criação de conquistas/badges, sistema de notificações de conquistas, interface de exibição.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 01/12/2025 a 09/12/2025
- **Observações:**

## SPRINT 18: Relatórios de Desempenho
- **Descrição:** Geração de relatórios detalhados de progresso, tempo de estudo, cursos concluídos, gráficos de desempenho.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 10/12/2025 a 19/12/2025
- **Observações:**

## SPRINT 19: Sistema de Busca e Filtros
- **Descrição:** Funcionalidade de busca por cursos, aulas, tags; filtros por categoria, avaliação, progresso.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 20/12/2025 a 26/12/2025
- **Observações:**

## SPRINT 20: Player de Vídeo Avançado
- **Descrição:** Funcionalidades avançadas do player (velocidade de reprodução, legendas, marcadores, fullscreen).
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 27/12/2025 a 05/01/2026
- **Observações:**

## SPRINT 21: Sistema de Backup e Sincronização
- **Descrição:** Funcionalidade para backup de dados, exportação/importação de progresso, sincronização entre dispositivos.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 06/01/2026 a 15/01/2026
- **Observações:**

## SPRINT 22: Configurações e Personalização
- **Descrição:** Painel de configurações da aplicação, temas, preferências de usuário, configurações de vídeo.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `1 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 16/01/2026 a 22/01/2026
- **Observações:**

## SPRINT 23: Otimização de Performance
- **Descrição:** Otimização de queries do banco, lazy loading, cache de dados, melhoria na inicialização da aplicação.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `5 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `1 dia deploy`.
- **Período:** 23/01/2026 a 01/02/2026
- **Observações:**

## SPRINT 24: Testes de Integração e Correções
- **Descrição:** Testes completos de todas as funcionalidades, correção de bugs, validação de fluxos de usuário.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `3 dias desenvolvimento` | `3 dia teste` | `2 dia revisão` | `1 dia deploy`.
- **Período:** 02/02/2026 a 11/02/2026
- **Observações:**

## SPRINT 25: Finalização e Polimento
- **Descrição:** Ajustes finais de interface, documentação do usuário, empacotamento final da aplicação, preparação para distribuição.
- **Desenvolvedor:** Jean Carlo Massing Jacintho.  
- **Cronograma:** `4 dias desenvolvimento` | `2 dia teste` | `1 dia revisão` | `2 dia deploy`.
- **Período:** 12/02/2026 a 21/02/2026
- **Observações:**
